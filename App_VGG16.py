...
MODEL_PATH = "C:/Users/gagan/OneDrive/Desktop/Projects/AIT736_Group_8_FinalProject/models/brain_tumor_detection_model_with_others.h5"
model = load_model(MODEL_PATH)

# Define confidence threshold for classifying as "not a brain MRI"
CONFIDENCE_THRESHOLD = 0.6

# Define class labels based on the order from model training
class_names = ['Others_AIT736', 'glioma', 'meningioma', 'notumor', 'pituitary']
...
def get_tumor_info(tumor_type):
    completion = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"Provide detailed information about {tumor_type} brain tumors."}
        ],
        temperature=0.7,
    )
...
# Function to format tumor information for HTML
...
def predict():
    ...
    # Check if it's "not a brain MRI" based on class and confidence
    if predicted_class_name == 'Others_AIT736' or confidence < CONFIDENCE_THRESHOLD:
        result_text = "This is not a brain MRI scan."
        tumor_info = "<p>The uploaded image does not appear to be a brain MRI scan.</p>"
    elif predicted_class_name == 'notumor':
